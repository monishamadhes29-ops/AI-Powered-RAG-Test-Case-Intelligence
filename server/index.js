...
console.log(
  `🔎 Langfuse config: host=${langfuseHost}, publicKey=${process.env.LANGFUSE_PUBLIC_KEY ? 'set' : 'MISSING'}, secretKey=${process.env.LANGFUSE_SECRET_KEY ? 'set' : 'MISSING'}`
);

// Every trace flush goes through here so success/failure/timeout is always logged
// the same way - a flush that silently hangs (e.g. blocked outbound network) would
// otherwise never print anything, making it look like tracing "isn't working".
async function flushLangfuseTrace(trace, { errorPath = false, timeoutMs = 10000 } = {}) {
  if (!trace) return;
  try {
    await Promise.race([
      langfuse.flushAsync(),
      new Promise((_, reject) => setTimeout(() => reject(new Error(`Langfuse flush timed out after ${timeoutMs}ms - check network access to ${langfuseHost}`)), timeoutMs))
    ]);
    console.log(`✅ Langfuse trace flushed${errorPath ? ' (error path)' : ''}:`, trace.id);
  } catch (flushError) {
    console.error('⚠️ Langfuse flush failed:', flushError.message);
  }
}
...
app.post('/api/search/deduplicate', async (req, res) => {
  let trace;
  try {
    const { results, threshold = 0.85, userId } = req.body;

    if (!results || !Array.isArray(results)) {
      return res.status(400).json({ error: 'Results array is required' });
    }

    trace = langfuse.trace({ name: 'search-deduplicate', userId, input: { resultCount: results.length, threshold } });
    const dedupSpan = trace.span({ name: 'deduplicate', input: { resultCount: results.length, threshold } });
    ...
    await flushLangfuseTrace(trace);
  } catch (error) {
    ...
    await flushLangfuseTrace(trace, { errorPath: true });
  }
});
...
app.post('/api/search/summarize', async (req, res) => {
  let trace;
  try {
    const { results, summaryType = 'concise', query = '', userId } = req.body;

    if (!results || !Array.isArray(results)) {
      return res.status(400).json({ error: 'Results array is required' });
    }

    trace = langfuse.trace({ name: 'search-summarize', userId, input: { query, summaryType, resultCount: results.length } });

    if (results.length === 0) {
      const emptyOutput = { summary: 'No results to summarize', resultCount: 0 };
      trace.update({ output: emptyOutput });
      const response = {
        success: true,
        summary: emptyOutput.summary,
        summaryType,
        resultCount: 0,
...
Provide detailed insights with specific examples from the test cases. Be thorough and technical.`
    ...
    const summarizationModel = process.env.GROQ_SUMMARIZATION_MODEL || 'llama-3.3-70b-versatile';

    const summarizeGeneration = trace.generation({
      name: 'summarize',
...
app.post('/api/test-prompt', async (req, res) => {
  let trace;
  try {
    const { prompt, temperature = 0.2, maxTokens = 4000, userId } = req.body;
    ...
    const promptModel = process.env.GROQ_RERANK_MODEL || 'llama-3.2-3b-preview';
    trace = langfuse.trace({ name: 'test-prompt', userId, input: { prompt, temperature, maxTokens } });
    const promptGeneration = trace.generation({ name: 'test-prompt', model: promptModel, input: prompt });
    ...
    trace.update({ output: parsedResponse });
...
// Reciprocal Rank Fusion: combines BM25 and vector rankings using each doc's rank
// position (not raw score), so no min-max normalization is needed and results are
// deterministic - no LLM call required.
function fuseWithRRF(bm25Results, vectorResults, limit, k = 60) {
  const map = new Map();

  bm25Results.forEach((doc, idx) => {
    map.set(doc.id, {
      ...doc,
      rrfScore: 1 / (k + idx + 1),
      bm25Rank: idx + 1,
  ...
  return Array.from(map.values())
    ...
    .map((doc, idx) => ({
      ...doc,
      rerankScore: doc.rrfScore,
      originalRank: doc.bm25Rank || doc.vectorRank,
      newRank: idx + 1
    }));
}

async function handleReranking(req, res, query, limit, filters, mode = 'groq') {
  ...
  try {
    ...
    bm25Pipeline.push(
      { $project: { _id: 1, id: { $ifNull: ['$id', '$testCaseId'] }, title: 1, description: 1, module: 1, priority: 1, risk: 1, steps: 1, expectedResults: 1, bm25Score: 1 } },
    ...
    vectorPipeline.push(
      { $project: { _id: 1, id: { $ifNull: ['$id', '$testCaseId'] }, title: 1, description: 1, module: 1, priority: 1, risk: 1, steps: 1, expectedResults: 1, vectorScore: 1 } }
    );
    ...
    if (mode === 'fusion') {
      // Deterministic Reciprocal Rank Fusion - no LLM call, works without a Groq key.
      console.log(`🔀 Fusing ${bm25Results.length} BM25 + ${vectorResults.length} vector candidates via RRF...`);
      const fusionStartTime = Date.now();

      const fused = fuseWithRRF(bm25Results, vectorResults, limit);

      const fusionTime = Date.now() - fusionStartTime;
      const totalTime = Date.now() - startTime;

      console.log(`✅ RRF fusion complete in ${fusionTime}ms`);
      ...
      return res.json({
        ...
        results: fused,
        count: fused.length,
        candidatesEvaluated: new Set([...bm25Results, ...vectorResults].map(d => d.id)).size,
        timing: {
          fusionTime,
          totalTime
        },
    ...
    // Merge results (deduplicate) for Groq reranking
...
app.post('/api/search', async (req, res) => {
  let trace;
  try {
    const { query, limit = 5, filters = {}, userId } = req.body;

    if (!query) {
      return res.status(400).json({ error: 'Query is required' });
    }

    trace = langfuse.trace({ name: 'search', userId, input: { query, filters } });
    ...
    const embeddingGeneration = trace.generation({
    ...
    const vectorSearchSpan = trace.span({ name: 'vector-search', input: pipeline });
    const results = await collection.aggregate(pipeline).toArray();
    vectorSearchSpan.end({ output: results });
...
app.post('/api/search/bm25', async (req, res) => {
  let trace;
  try {
    const { query, limit = 10, filters = {}, fields = ['id', 'title', 'description', 'steps', 'expectedResults', 'module'], userId } = req.body;
    ...
    trace = langfuse.trace({ name: 'search-bm25', userId, input: { query, filters, fields } });
    ...
    const startTime = Date.now();
    const bm25Span = trace.span({ name: 'bm25-search', input: pipeline });
    const results = await collection.aggregate(pipeline).toArray();
    bm25Span.end({ output: results });
...
app.post('/api/search/hybrid', async (req, res) => {
  ...
  try {
    ...
    trace = langfuse.trace({ name: 'search-hybrid', userId, input: { query, filters, weights: { bm25Weight, vectorWeight } } });
    ...
    const bm25Span = trace.span({ name: 'bm25-search', input: bm25Pipeline });
    const bm25Results = await collection.aggregate(bm25Pipeline).toArray();
    bm25Span.end({ output: bm25Results });
    ...
    embeddingGeneration.end({
      output: { tokens, cost: embeddingCost },
      usage: { totalTokens: tokens },
    });
    ...
    const vectorSpan = trace.span({ name: 'vector-search', input: vectorPipeline });
    const vectorResults = await collection.aggregate(vectorPipeline).toArray();
    vectorSpan.end({ output: vectorResults });
    const vectorTime = Date.now() - vectorStartTime;

    // 3. Normalize and combine scores
    console.log('🔀 Combining results...');
    const fusionSpan = trace.span({ name: 'score-fusion', input: { bm25Weight, vectorWeight } });
    ...
    fusionSpan.end({ output: { stats: { foundInBoth: bothCount, foundInBm25Only: bm25OnlyCount, foundInVectorOnly: vectorOnlyCount }, count: finalResults.length } });
    trace.update({ output: finalResults });
...
app.post('/api/search/rerank', async (req, res) => {
  try {
    ...
    // Score-fusion mode is deterministic RRF over BM25+vector ranks - no LLM call, no Groq key needed.
    if (mode === 'groq') {
      if (!process.env.GROQ_API_KEY || process.env.GROQ_API_KEY === 'your_groq_api_key_here') {
        return res.status(400).json({
    ...
    return handleReranking(req, res, query, limit, filters, mode);
...
app.listen(PORT, () => {
  ...
  // One-time connectivity check so a blocked/misconfigured Langfuse host is caught
  // at boot instead of silently swallowing every trace flush during requests.
  const startupTrace = langfuse.trace({ name: 'server-startup-check', input: { pid: process.pid } });
  startupTrace.update({ output: { ok: true } });
  flushLangfuseTrace(startupTrace).then(() => {
    console.log('🔎 Langfuse startup check complete - if you did not see a "flushed" line above, tracing is not reaching', langfuseHost);
  });
});