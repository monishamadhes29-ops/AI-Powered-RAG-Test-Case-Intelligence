...
import { expandAbbreviationsContextual, smartExpand } from './abbreviationMapper.js';
...
export function preprocessQueryComplete(rawQuery, options = {}) {
  ...
  const abbrevResult = expandAbbreviationsContextual(normalized, { customMap: customAbbreviations });
...
export function analyzeQuery(rawQuery) {
  ...
  const abbrevResult = expandAbbreviationsContextual(normalized);
...
export function preprocessQueryWithLogs(rawQuery, options = {}) {
  ...
  // Abbreviations
  const t2 = Date.now();
  const abbrevResult = expandAbbreviationsContextual(normalizeResult.normalized);
  logs.push({
    step: 'abbreviations',
    ...
    ambiguous: abbrevResult.ambiguous || []
  });
