...
function RerankingSearch() {
  ...
  const methodLabel = isFusion ? 'RRF Score Fusion' : 'Groq AI Semantic Reranking';
  const rerankScoreLabel = isFusion ? 'RRF Score' : 'Groq Rerank';
    ...
    try {
      ...
      if (data.success) {
        ...
        setSearchInfo({
          ...
          searchTime: data.timing?.groqTime || data.timing?.fusionTime || 0,
          rerankingTime: data.timing?.groqTime || data.timing?.fusionTime || 0,
        ...
        enqueueSnackbar(`Found ${data.count} results ranked by ${isFusion ? 'RRF Score Fusion' : 'Groq AI'}`, { variant: 'success' });
      }
    } catch (err) {
    ...
    <Box sx={{ maxWidth: 1400, margin: 'auto', padding: 3 }}>
      <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Combines BM25 keyword search and Vector semantic search, then reranks candidates via {methodLabel}.
        </Typography>
        ...
        <Paper variant="outlined" sx={{ p: 2, mt: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            ...
            <Tooltip title="Groq AI asks an LLM to score each candidate (best quality, needs a Groq API key). Score Fusion combines BM25 and vector ranks deterministically via Reciprocal Rank Fusion (fast, free, no LLM call).">
              <IconButton size="small" onClick={() => setShowModeInfo(!showModeInfo)}>
                <InfoIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          </Box>

          <Collapse in={showModeInfo}>
            <Alert severity="info" sx={{ mb: 2 }}>
              <Typography variant="body2">
                <strong>Groq AI (LLM):</strong> Semantic relevance scoring via a Groq-hosted LLM. Highest quality, requires <code>GROQ_API_KEY</code>.<br />
                <strong>Score Fusion (RRF):</strong> Reciprocal Rank Fusion over BM25 + vector rank positions. Deterministic, no LLM call or API key needed.
              </Typography>
            </Alert>
          </Collapse>
          ...
          <ToggleButtonGroup
            ...
            size="small"
          >
            <ToggleButton value="groq">Groq AI (LLM)</ToggleButton>
            <ToggleButton value="fusion">Score Fusion (RRF)</ToggleButton>
          </ToggleButtonGroup>
        </Paper>
      ...
      {searchInfo && (
        ...
        >
          <Typography variant="body2">
            <strong>Reranking Results:</strong> Found {searchInfo.count} test cases ranked by {isFusion ? 'RRF Score Fusion' : 'Groq AI'}
            <br />
            <strong>Timing:</strong> Total: {searchInfo.totalTime}ms
            <br />
            <strong>Method:</strong> BM25 + Vector → {methodLabel}
          </Typography>
        </Alert>
      )}
        ...
        <Box>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Paper variant="outlined" sx={{ p: 1, bgcolor: 'grey.50' }}>
                          ...
                          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                            ...
                            <Typography variant="caption" color="text.secondary">
                              ⭐ {rerankScoreLabel}: {formatScore(result.rerankScore || 0.5)}
                            </Typography>
                          </Box>
                        </Paper>
                      </Box>
          ...
          {tabValue === 1 && (
            <Box>
              <Typography variant="h6" sx={{ mb: 2 }}>
                <ArticleIcon sx={{ verticalAlign: 'middle', mr: 1 }} />
                Initial Candidate Ranking (Before {methodLabel} - {beforeResults.length} results from 50 candidates)
              </Typography>

              <Alert severity="warning" sx={{ mb: 2 }}>
                <Typography variant="body2">
                  These are the top candidates from the 50-candidate pool before {methodLabel}. Notice how reranking reorders them based on actual relevance to your query!
                </Typography>
              </Alert>
                      ...
                      <Box sx={{ flex: 1 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, flexWrap: 'wrap' }}>
                          ...
                          {result.rerankScore !== undefined && (
                            <Chip
                              label={`${rerankScoreLabel}: ${formatScore(result.rerankScore)}`}
                              size="small"
                              color="success"
                              sx={{ fontWeight: 'bold' }}
                            />
            ...
            <Box>
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell><strong>{rerankScoreLabel}</strong></TableCell>
              ...
              <Alert severity="info" icon={<InfoIcon />} sx={{ mt: 2 }}>
                <Typography variant="body2">
                  ...
                  • <strong>{rerankScoreLabel}:</strong> {isFusion ? 'Reciprocal Rank Fusion score' : 'AI-based relevance score'}<br />
                  • <strong>Final Rank:</strong> Top-10 position after reranking<br />
                  • 🟦 Highlighted rows = Top-3 results after reranking<br />
                  • Compare Original Rank vs Final Rank to see how much reranking reordered each result
                </Typography>
              </Alert>
            </Box>
          )}
